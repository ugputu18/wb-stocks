...
13. [Перераспределение WB (read-side)](#13-перераспределение-wb-read-side)
...
**Таблица `wb_region_macro_region`:** явное сопоставление `region_key` → регион; строки БД перекрывают bootstrap в **`wbRegionKeyMacroRegionBootstrap.ts`**. Без маппинга buyer-регион попадает в «Не сопоставлен». На **`/redistribution`** по умолчанию **regional ranking**: **`transferScore`** использует **`targetRegionalDemand`** (buyer-region, Σ по региону цели); опционально **fulfillment** — по **`targetForecastDailyDemand`**. Донор и **`recommendedToWB`** складовые. Заполняется вручную (SQL/seed) при необходимости, без эвристики по подстроке.
...
**Локальный forecast UI** (`pnpm serve:forecast-ui`, см. `docs/forecast-ui.md`): открывать **`/`** (Preact, `pnpm build:forecast-ui-client`); старый vanilla-экран на `/legacy` и `/static/*` удалён. Чтение `wb_forecast_snapshots` + `own_stock_snapshots`, pipeline не трогаем.

**Операционные страницы forecast UI:** для интерфейсов вроде региональных запасов,
планирования заказа у производителя и будущих «что делать сейчас» экранов **не
даём оператору выбирать дату среза**. Страница/API должны брать самый свежий
доступный `snapshot_date` автоматически, явно показывать в основном блоке
управления/меню, на какую дату показана информация, и давать действие
**«Обновить»** для повторной загрузки последнего среза. Исторический выбор даты
допустим только в диагностических/аналитических экранах, где сравнение прошлых
срезов является самой задачей.

**Стили и примитивы forecast-ui-client (поэтапная миграция):** в клиенте подключены **Panda CSS** (атомарный CSS на этапе сборки, recipes/design tokens) и **Radix UI** (`@radix-ui/react-popover`; в Vite алиасы `react` / `react-dom` → `preact/compat`). Конфиг Panda: **`forecast-ui-client/panda.config.ts`**, слой входа: **`src/panda.css`**, PostCSS: **`forecast-ui-client/postcss.config.cjs`** (явный `configPath`). Скрипт **`pnpm run panda:forecast-ui`** выполняет `panda codegen` + `panda cssgen`; он вызывается из **`prepare`** и перед **`dev:forecast-ui-client`** / **`build:forecast-ui-client`**. Сгенерированный каталог **`forecast-ui-client/styled-system/`** в **`.gitignore`** — пересоздаётся при установке и сборке. Переиспользуемые примитивы: **`forecast-ui-client/src/components/ui/`** (`Panel`, `SectionHeading`, `Badge`, `FieldHint`, `InlineAction`, `PopoverInfo`, алиас `Disclosure`). Уже переведены на Panda + Radix: **`FiltersForm`** (строки фильтров — **`filterFormRow`** (**`primary`** / **`secondary`**) + колонки **`filterField`** с вариантами **`layout`**, быстрый фокус — **`quickFiltersBar`** / **`quickFilterChip`**, блок параметров расчёта — **`calcParams*`**; все подписи полей через **`LabelWithInlineHelp`** (**`fieldLabelRow`**), где нужен текст помощи — **`HelpToggle`** (**`helpTrigger` / `helpPopover`**)), **`RegionWarehousesDisclosure`** (тот же **`helpTrigger`**), секция **`RedistributionControls`** (обёртка **`Panel`**), блок **рекомендаций** на **`/redistribution`** (**`RedistributionResultsSection`**: **`Panel`**, **`SectionHeading`**, таблица через **`DenseDataTable`** / **`ScrollTableWrap`**, pill режима ranking, строки и ячейка региона — recipes + **`css()`** для состояний строк; **`table-wrap`** из темы для той же рамки, что у остальных таблиц), **`SupplierTable`** / **`MainTable`** (пустое состояние — recipe **`tableEmptyState`**, тулбар фильтров supplier — **`toolbarRow`**). Стили главной forecast-страницы (**сводка KPI**, **detail diagnosis** и т.д.; мост **`.controls label.fu-filter-field`** для gap) в **`forecast-ui-client/src/pages/forecast-page.css`**, подключение из **`main.tsx`**, чтобы **`.forecast-next-root`** и общие правила применялись ко всем маршрутам SPA. Остальной UI пока на существующих листах (**`forecast-ui-theme.css`**, **`hints.css`**, частично **`redistribution-page.css`** и т.д.).

**Регионы складов WB** в forecast UI: явный mapping `warehouse_key` → кластер в **`wb-stocks/src/domain/wbWarehouseMacroRegion.ts`** (клиент реэкспортирует); без эвристики по имени; неизвестный склад → подпись **«Не сопоставлен»**. Диагностика: **`GET /warehouse-region-audit`** в UI и **`GET /api/forecast/warehouse-region-audit`** (агрегаты по `wb_forecast_snapshots`, список складов без маппинга по спросу). Таблица **`wb_region_macro_region`** относится к **регионам заказов** (нормализованный `regionName`), отдельно от складского справочника; для **regional**-ranking на **`/redistribution`** склад и buyer-регион сводятся к **одному имени региона** для сигнала сортировки.

**Кастомные пути forecast UI (SPA, один `index.html`):** **`/`**, **`/redistribution`**, **`/regional-stocks`**, **`/manufacturer-order`**, **`/warehouse-region-audit`**, **`/regional-demand-diagnostics`** — в клиенте импорт из **`forecast-ui-client/src/routes.ts`**; реализация общая с сервером в **`src/forecastUiRoutes.ts`**.

**Региональный спрос (read-side):** **`wb_orders_daily_by_region`** и **`wb_region_demand_snapshots`** — спрос по **`regionName`** из заказов WB (не по складу исполнения). Поле **`regional_forecast_daily_demand`** — аналог `forecast_daily_demand` в `wb_demand_snapshots`. Сверка по SKU: **`GET /api/forecast/regional-demand-verify?snapshotDate=&nmId=&techSize=`**; **`POST /api/forecast/regional-demand`** — пакетно по SKU (в JSON — полный merged **`regionMacroMap`**: bootstrap + БД). **Сводка по всей сети (без SKU):** страница **`/regional-demand-diagnostics`**, API **`GET /api/forecast/regional-vs-warehouse-summary`** — сравнение regional vs fulfillment по регионам; расширенный явный маппинг субъектов РФ в bootstrap снижает долю unmapped и делает **`comparisonByMacroRegion`** содержательнее; блок unmapped остаётся для контроля. Основной прогноз и главная таблица по-прежнему на **fulfillment**; **`/redistribution`** для приоритета перевозок по умолчанию **regional** (buyer-region signal), **`rankingMode=fulfillment`** — альтернатива по складу исполнения.

**Сырые заказы WB (диагностика, read-only, без записи в БД):** построчно в SQLite не храним — только дневные агрегаты. Для проверки **`regionName` (buyer) vs `warehouseName` (исполнение)** в forecast UI добавлены **`GET /api/forecast/raw-orders-diagnostics`**, **`GET /api/forecast/order-flow-by-region`**, **`GET /api/forecast/order-flow-macro-matrix`** (параметры **`dateFrom`**, **`dateTo`**, опционально **`nmId`**, **`vendorCode`**; окно до 31 дня; нужен **`WB_TOKEN`** — live-вызов WB Statistics API). Матрица регионов: buyer → **`wbRegionMacroRegion`**, fulfillment → **`wbWarehouseMacroRegion`**. Подробности — `docs/forecast-ui.md`, раздел «Диагностика: сырые заказы WB».

**Перемещение между складами WB (`/redistribution`).** Read-side эвристика на снимке прогноза: донор всегда склад; по умолчанию цель — макрорегион buyer-demand (**regional**), альтернатива — цель = склад исполнения (**fulfillment**, query `rankingMode=fulfillment`). Продуктовая модель, реестр складов, совместимость макрорегионов, skip донор/цель, честные строки без кандидатов — **[§13](#13-перераспределение-wb-read-side)** и [`docs/redistribution-product.md`](./docs/redistribution-product.md), [`docs/redistribution-read-model.md`](./docs/redistribution-read-model.md). Пошаговые формулы, запросы API, сводка донора, «Товары донора», inline-сеть по SKU — [`docs/forecast-ui.md`](./docs/forecast-ui.md), раздел «Перемещение между складами WB».

**Основная таблица** переключается query **`viewMode`** (по умолчанию **`systemTotal`**): **`systemTotal`** («Запасы в целом») — одна строка на SKU, риск и дни запаса по пулу **system** (WB∑+own), колонка **OOS (system)** — read-side оценка **`snapshot_date + floor(daysOfStockSystem)`** календарных дней при положительном Σ-спросе (согласована с «Дн. system»; не `MIN(stockout_date)` по WB); **`wbTotal`** («WB в целом») — та же строка SKU, бакет риска по **WB**, OOS — **`MIN(stockout_date)`** по сети; либо **`wbWarehouses`** (строка = `warehouse_key × sku`). В JSON для агрегатов и supplier-витрины read-side отдаёт **`wbStartStockTotal`** / **`wbIncomingUnitsTotal`**: раскладка **`wbAvailableTotal`** (= сток + в пути по сети WB для SKU). Рекомендации «На WB» и supplier-план те же read-side формулы, не дублируя business logic. Из режимов **`wbTotal`** / **`systemTotal`** можно одним действием перейти к строкам по складам: UI выставляет **`viewMode=wbWarehouses`**, **`q=<nm_id>`** и опционально **`techSize`** (узкий фильтр, только при числовом `q`). Фильтры и режим **дублируются в адресной строке** (`history.replaceState` после загрузки; drilldown через `pushState`; восстановление при refresh и **Назад**); токен UI в URL **не** пишется. **Supplier replenishment** остаётся **sku-level** (`nm_id` + `tech_size`): `GET /api/forecast/supplier-replenishment` — **одна строка на SKU**; «Заказать» от пула (`recommendedFromSupplier`), плюс read-side **план заказа с lead time** — `leadTimeDays` / `coverageDays` / `safetyDays`, поля `recommendedOrderQty`, `daysUntilStockout`. В **`GET /api/forecast/summary`** KPI по рискам и **`recommendedToWBTotal`** следуют выбранному **`viewMode`**; **`recommendedFromSupplierTotal`** и **`recommendedOrderQtyTotal`** — всегда по SKU-витрине. Экспорт: `GET /api/forecast/export-wb` (ветвление колонок по `viewMode`), `GET /api/forecast/export-supplier`. В панели **«Детали строки»** основной UI показывает **расшифровку формул** рекомендации «На WB» и заказа у поставщика (см. `docs/forecast-ui.md`, раздел «Расшифровка рекомендаций»). **Основной UI** (`forecast-ui-client`): общие примитивы подсказок в `components/hints/` и явное отображение **сток / в пути** в таблицах и explain (см. `docs/forecast-ui.md`, «Основной UI»).

---

## 13. Перераспределение WB (read-side)

Страница **`/redistribution`** в forecast UI не меняет pipeline прогноза: только читает `wb_forecast_snapshots`, догружает сеть по SKU и при regional-режиме — пакет регионального спроса (`POST /api/forecast/regional-demand`).
