...
Query-параметры (в дополнение к базовым): **`viewMode`**, **`targetCoverageDays`**, **`replenishmentMode`**, **`ownWarehouseCode`**, **`leadTimeDays`**, **`coverageDays`**, **`safetyDays`**.
...
### `GET /api/forecast/rows?...&viewMode=&riskStockout=&targetCoverageDays=&replenishmentMode=&ownWarehouseCode=&leadTimeDays=&coverageDays=&safetyDays=`
