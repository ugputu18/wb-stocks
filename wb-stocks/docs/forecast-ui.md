...
- **`GET /api/forecast/*`** (health, warehouse-keys, rows, summary, supplier-replenishment, export XLSX) читают только SQLite — **`WB_TOKEN` не нужен**.
...
Кнопки **Скачать WB Excel** / **Скачать Supplier Excel** вызывают `GET /api/forecast/export-wb` и `GET /api/forecast/export-supplier` (те же фильтры, что и у таблиц; supplier-выгрузка требует явный `targetCoverageDays` в query). Файлы отдаются как XLSX — числа сохраняются числовыми ячейками и не зависят от локали (исторически здесь был CSV, но Excel ломал десятичную точку и длинные `nm_id`).
...
XLSX **на сервере** (`Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`), имя файла `wb-replenishment-<date>-h<horizon>.xlsx` / `supplier-replenishment-<date>-h<horizon>.xlsx`. Имена с кириллическими частями (например, регион в `regional-stocks-Центральный-…`) проставляются и в `filename=` (ASCII-фоллбэк) и в RFC 5987 `filename*=UTF-8''…` — современные браузеры берут оригинал.
...
- **export-supplier** — обязателен **`targetCoverageDays`** в query; остальное как у `/supplier-replenishment` (включая `leadTimeDays`, `coverageDays`, `safetyDays`, опционально `viewMode` для единообразия URL — на состав SKU не влияет). В выгрузке есть **`wb_start_stock_total`** и **`wb_incoming_units_total`** перед **`wb_available_total`**.

Числовые поля (`nm_id`, `days_of_stock_*`, `recommended_*` и др.) пишутся как настоящие XLSX-числа, поэтому Excel/LibreOffice не пытается их интерпретировать по локали; шапка совпадает с CSV-выгрузкой, которая была здесь раньше (порядок и имена колонок не меняли — это контракт для аналитических шаблонов).
