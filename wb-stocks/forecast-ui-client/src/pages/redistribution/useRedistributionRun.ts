      ...
      if (recs.length === 0) {
        const emptyMsg =
          effectiveRankingMode === "regional"
            ? "Расчёт выполнен, но нет направлений: нет макрорегионов с дефицитом до целевого покрытия (с учётом Σ local в регионе), цель совпадает с макрорегионом донора, либо излишек донора меньше минимума после нехватки."
