...
describe("forecast UI routes (shared with server re-export)", () => {
  it("normalizes pathnames", () => {
    ...
    expect(normalizeForecastUiPathname("/service/")).toBe(SERVICE_ROUTE);
  ...
  it("isKnownForecastRoute / isForecastUiSpaPath match SPA roots", () => {
    ...
    expect(isKnownForecastRoute("/service")).toBe(true);
  ...
  it("registry constants stay stable", () => {
    ...
    expect(SERVICE_ROUTE).toBe("/service");
    expect(FORECAST_UI_SPA_ROUTES.service).toBe(SERVICE_ROUTE);
